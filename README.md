...
- Наблюдения должны быть независимы друг от друга (здесь "независимы" означает отсутствие парности. Совокупности не должны, например, отражать состояние до и после)
- Переменные должны быть категориями
- Минимальная численность категорий - 5 для 80% наблюдений
```Python
